/* ============================================================================
   ПРАВКА [ADV] — авансы («предоплаты») бонусов в маржинальной прибыли.
   Дата: 26.08.2026. Автор правки: Claude по задаче Е. Ганюшиной.

   ПРОБЛЕМА
   Аванс бонуса фрилансеру приходит из 1С в salary_by_user.man_salary_all
   отрицательным начислением в месяце зачёта (положительной проводки в месяце
   выплаты нет). fn_salary_for_all_user кладёт этот минус в manzp, дальше он
   попадает в allsumm и УМЕНЬШАЕТ затраты отдела — маржа и бонус БДМ завышены.

   РЕШЕНИЕ (по требованию бизнеса)
   Выгрузку из 1С не меняем, новых полей в отчёт не добавляем.
   В отчёте выплат минус остаётся как есть (сотрудник должен отработать).
   В расчёт ФОТ отрицательная строка входит ПО МОДУЛЮ: сумма долга увеличивает
   затраты отдела, а не уменьшает их. Изменены ровно две строки — суммирование
   allsumm_with_coef обёрнуто в ABS(); всё остальное тело функций не тронуто.

   ДЕЙСТВУЕТ НА ВСЕ ПЕРИОДЫ, включая закрытые (решение согласовано).
   Затронутые данные (все отрицательные строки salary_by_user, все — Pharma):
     11.2024 Гулюкина    -22 101      итог строки  +70 779     — не меняется
     11.2025 Дашковская -123 379,20   итог строки +120 153,60  — не меняется
     11.2025 Беликова   -100 000      итог строки  -65 440     — МЕНЯЕТСЯ
     12.2025 Беликова    -50 000      итог строки  +10 000     — не меняется
     07.2026 Ревенкова  -100 000      итог строки  -62 622,40  — МЕНЯЕТСЯ

   ОЖИДАЕМЫЙ РЕЗУЛЬТАТ (проверено симуляцией до применения):
     Pharma, июль 2026:  затраты 1 535 362,65 -> 1 660 607,45
                         маржа   1 528 117,35 -> 1 402 872,55
     Pharma, ноябрь 2025: затраты  635 117,20 ->  765 997,20
                          маржа    697 778,80 ->  566 898,80
     Pharma, Q3 2026:    маржа  3 392 801,76 -> 3 267 556,96 (бонус БДМ 14 %:
                         474 992,25 -> ~457 458)
     Pharma, Q4 2025:    маржа  2 448 573,19 -> 2 317 693,19 (бонус БДМ 5 %:
                         122 428,66 -> ~115 885) — квартал закрыт, выплаченный
                         бонус БДМ не пересчитываем, отчёт покажет новую цифру.
   Другие отделы и другие периоды правкой не затрагиваются.

   ОТКАТ: rollback_margin_abs_advance.sql (точные снимки версий на 26.08.2026).
   ============================================================================ */
    ...
    from (
        ...
        from (
            select
                ...
                salary_month,
                salary_year,
                sum(ABS(allsumm_with_coef)) as sum_salary_depatment,  -- [ADV] аванс: отрицательная строка входит в ФОТ по модулю
...
BEGIN
    ...
    DECLARE cur1 CURSOR FOR (
        ...
        FROM (
            ...
            FROM (
                SELECT DISTINCT
                    SUM(ABS(allsumm_with_coef)) OVER (  -- [ADV] аванс: отрицательная строка входит в ФОТ по модулю
