/*
  dbo.fn_salary_for_all_user — полный текст функции с правкой от 20.08.2026.

  Функция питает вкладки «Помесячно», «Поквартально» и «Выплаты», а через
  fn_marginality_by_month / fn_margin_bonus_by_quarter — расчёт маржинальной прибыли.

  ЧТО ИЗМЕНЕНО (единственная правка, две строки):
    в расчёт manzp для НЕ-BDM должностей добавлен вычет "- fu.man_project_bonus"
    (обе ветки: main_department = 1 и остальные). У BDM (pos_id = 7) этот вычет
    был и раньше. Теперь проектный бонус БДМ не попадает в зарплату ни у кого,
    а значит не входит ни в allsumm, ни в затраты отдела при расчёте маржи.

    НДФЛ по-прежнему вычитается из manzp во всех ветках — как и было.

  Эффект (июль 2026, отдел Industrial, Сидоренкова К.А., проектный бонус 20 400):
    Зарплата                    89 299,00 -> 68 899,00
    Вся сумма без коэффициента  94 099,00 -> 73 699,00
    Вся сумма                  131 738,60 -> 103 178,60
    Затраты отдела             488 675,41 -> 460 115,41
    Маржа отдела               202 168,73 -> 230 728,73
  Других строк с man_project_bonus у не-BDM должностей в salary_by_user нет
  (проверено за 2024-2026), другие периоды правка не затрагивает.

  Остальное тело функции оставлено без изменений — это дословная копия того,
  что сейчас в базе (снимок: fn_salary_for_all_user_BEFORE.sql).
*/

ALTER FUNCTION [dbo].[fn_salary_for_all_user]
(
  @date1 DATETIME,
  @date2 datetime,
  @department_id int = 0
                        ...
                        CASE
                            WHEN fu.main_department = 1 THEN
                                ((fu.man_salary_all - fu.man_bonus - fu.man_salary_ndfl -
                                  ...
                                  fu.salary_sick_days_pay - fu.salary_vacation_pay - fu.man_project_bonus)
                                  * ISNULL(fu.[percent], 100) / 100)
