/*
  СНИМОК ИСХОДНОЙ ВЕРСИИ функции dbo.fn_salary_for_all_user_sverka —
  такой она была до правки от 20.08.2026 10:47 (вычет отпускных и больничных).
  Текст снят через OBJECT_DEFINITION в начале сессии, приведён дословно.

  Отличие от текущей версии в БД: в расчёте manzp (обе ветки case) НЕТ вычета
  salary_vacation_pay и salary_sick_days_pay.

  Чтобы откатиться на эту версию — замените ниже CREATE FUNCTION на ALTER FUNCTION
  и выполните скрипт.
*/
