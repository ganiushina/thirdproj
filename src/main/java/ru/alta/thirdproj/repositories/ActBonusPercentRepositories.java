...
public class ActBonusPercentRepositories {
    ...
    private static final String SELECT_ACT_EXTRA_QUERY = "SELECT distinct ab.id act_id, isnull(ebx.bonus_percent, '') per, CASE WHEN isnull(ebx.act_id, 0) = 0 then 0 ELSE 1 end paied,\n" +
            "ab.total_no_nds, ab.company_name, ab.candidate FROM dbo.act_buh ab\n" +
            "LEFT JOIN (SELECT eb.act_id, eb.bonus_percent FROM extra_bonus eb \n" +
            "\t\t\tJOIN act_buh ab ON ab.id= eb.act_id\n" +
            "\t\t\tWHERE eb.employer_id = :employer_id AND ab.date_act BETWEEN :date1 AND :date2) ebx ON ebx.act_id = ab.id\n" +
    ...
    public List<ExtraAct> getAllExtraActs(LocalDate date1, LocalDate date2, Integer employerId) {
        try (Connection connection = sql2o.open()) {
            Query query = connection.createQuery(SELECT_ACT_QUERY, false)
                    .addParameter("date1", date1)
                    .addParameter("date2", date2)
            ...
            List<ExtraAct> extraActList = new ArrayList<>();
            ...
            for (Map<String, Object> row : list) {
                ...
                for (var entry : row.entrySet()) {
                    ...
                    switch (key) {
                        case "act_id":
                            extraAct.setActId((Integer) value);
                            break;
                        case "total_no_nds":
                            extraAct.setTotalSum((Double) value);
                            // Форматирование суммы, если нужно
                            // String formatted = currencyInstance.format(value);
                            break;
                        case "per":
                            extraAct.setPercent((Double) value);
                            break;
                        case "paied":
                            extraAct.setPaid((Integer) value);
                            break;
                        case "company_name":
                            extraAct.setCompany((String) value);
                            break;
                        case "candidate":
                            extraAct.setCandidate((String) value);
                            break;
                    }
                }
                extraActList.add(extraAct);
            }
            return extraActList;
        }
    }
